...
// ============================================================
// === FUNGSI: SSRF PROTECTION UNTUK OUTBOUND WEBHOOKS ===
// ============================================================
        ...
        if (Array.isArray(address)) {
            for (const addr of address) {
                if (isPrivateIP(addr.address)) {
                    return callback(new Error(`SSRF blocked: Private IP detected (${addr.address})`));
                }
            }
        } else {
            if (isPrivateIP(address)) {
                return callback(new Error(`SSRF blocked: Private IP detected (${address})`));
            }
        }
...
async function checkPendingTransactions() {
    ...
    for (const tx of pendingTx) {
        ...
        if (committedStatus && tx.h2h_callback_url) {
            ...
            axios.post(tx.h2h_callback_url, webhookPayload, { timeout: PROVIDER_TIMEOUT_MS, httpAgent: safeHttpAgent, httpsAgent: safeHttpsAgent })
