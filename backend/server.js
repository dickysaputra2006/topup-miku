...
app.post('/api/admin/balance/add', protectAdmin, async (req, res) => {
    ...
    try {
        ...
        await client.query(historySql, [user.id, parseInt(amount), 'Deposit', description, `ADMIN_ADD-${Date.now()}-${crypto.randomBytes(8).toString('hex')}`]);
        await client.query('COMMIT');
...
app.post('/api/admin/balance/reduce', protectAdmin, async (req, res) => {
    ...
    try {
        ...
        const historySql = 'INSERT INTO balance_history (user_id, amount, type, description, reference_id) VALUES ($1, $2, $3, $4, $5)';
        await client.query(historySql, [user.id, -amount, 'Refund', description, `ADMIN_REDUCE-${Date.now()}-${crypto.randomBytes(8).toString('hex')}`]);
        await client.query('COMMIT');
...
app.post('/api/order', protect, async (req, res) => {
    ...
    try {
        ...
        const invoiceId = `TRX-${Date.now()}-${userId}-${crypto.randomBytes(8).toString('hex')}`;
...
app.post('/h2h/order', protectH2HIp, protectH2H, async (req, res) => {
    ...
    try {
        ...
        const invoiceId = `H2H-${Date.now()}-${h2hUser.id}-${crypto.randomBytes(8).toString('hex')}`;
