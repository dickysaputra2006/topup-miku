...
app.post('/api/admin/balance/add', protectAdmin, async (req, res) => {
    ...
    try {
        ...
        await client.query(historySql, [user.id, parseInt(amount), 'Deposit', description, `ADMIN_ADD-${crypto.randomBytes(8).toString('hex')}`]);
        await client.query('COMMIT');
...
app.post('/api/admin/balance/reduce', protectAdmin, async (req, res) => {
    ...
    try {
        ...
        const historySql = 'INSERT INTO balance_history (user_id, amount, type, description, reference_id) VALUES ($1, $2, $3, $4, $5)';
        await client.query(historySql, [user.id, -amount, 'Refund', description, `ADMIN_REDUCE-${crypto.randomBytes(8).toString('hex')}`]);
        await client.query('COMMIT');
...
app.post('/api/order', protect, async (req, res) => {
    ...
    try {
        ...
        const invoiceId = `TRX-${crypto.randomBytes(8).toString('hex')}${userId}`;
        const trx_id_provider = `WEB-${crypto.randomBytes(8).toString('hex')}`;
...
app.post('/h2h/order', protectH2HIp, protectH2H, async (req, res) => {
    ...
    try {
        ...
        const invoiceId = `H2H-${crypto.randomBytes(8).toString('hex')}${h2hUser.id}`;
        const trx_id_provider = `H2H-PROVIDER-${crypto.randomBytes(8).toString('hex')}`;
