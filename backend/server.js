...
app.post('/api/deposit/request', protect, async (req, res) => {
    ...
    try {
        ...
        // Sentinel: Use crypto.randomInt for cryptographically secure random number generation
...
app.post('/api/order', protect, async (req, res) => {
    ...
    try {
        ...
        // Sentinel: Ensure transaction IDs are unguessable to prevent callback forgery
        const cryptoSuffix = crypto.randomBytes(4).toString('hex');
        const invoiceId = `TRX-${Date.now()}${userId}-${cryptoSuffix}`;
        const trx_id_provider = `WEB-${Date.now()}-${cryptoSuffix}`;
...
app.post('/h2h/order', protectH2HIp, protectH2H, async (req, res) => {
    ...
    try {
        ...
        // Sentinel: Ensure H2H transaction IDs are unguessable to prevent callback forgery
        const cryptoSuffix = crypto.randomBytes(4).toString('hex');
        const invoiceId = `H2H-${Date.now()}${h2hUser.id}-${cryptoSuffix}`;
        const trx_id_provider = `H2H-PROVIDER-${Date.now()}-${cryptoSuffix}`;
