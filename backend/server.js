...
const app = express();
const PORT = 3000;
const JWT_SECRET = 'kunci-rahasia-yang-sangat-aman-untuk-proyek-ini';
...
app.post('/api/admin/sync-products', protectAdmin, async (req, res) => {
    ...
    try {
        await connection.beginTransaction();
        const response = await axios.get('http://localhost:4000/foxy-api/products');
        const providerProducts = response.data;
        for (const product of providerProducts) {
            let [games] = await connection.query('SELECT id FROM games WHERE name = ?', [product.game]);
            if (games.length === 0) {
                console.warn(`Game "${product.game}" tidak ditemukan. Produk dilewati.`);
                continue;
            }
            ...
            const sellingPrice = product.base_price * (1 + (margin_percent / 100));
            ...
            await connection.query(sql, [gameId, categoryId, product.name, product.sku, sellingPrice]);
        }
        await connection.commit();
        ...
        res.status(500).json({ message: 'Gagal melakukan sinkronisasi.' });
    } finally {
        connection.release();
    }
});
...
app.post('/api/order', protect, async (req, res) => {
    ...
    try {
        ...
        const finalTarget = product.needs_server_id ? `${targetGameId}|${targetServerId}` : targetGameId;
        ...
        await connection.query(txSql, [invoiceId, userId, productId, finalTarget, finalPrice, trx_id_provider]);
        ...
        const foxyPayload = { product_code: product.provider_sku, user_id: targetGameId, server_id: targetServerId || '', trx_id: trx_id_provider, callback_url: `http://localhost:3000/api/foxy/callback` };
        axios.post(`${FOXY_BASE_URL}/order`, foxyPayload, { headers: { 'Authorization': FOXY_API_KEY, 'Content-Type': 'application/json' } }).catch(err => { console.error("Foxy API Error on initial order:", err.response ? err.response.data : err.message); });
        await connection.commit();
...
app.post('/api/foxy/callback', async (req, res) => {
    ...
    try {
        ...
        // Ambil transaksi berdasarkan invoice ID
        const [transactions] = await connection.query('SELECT * FROM transactions WHERE invoice_id = ? AND status = "Pending" FOR UPDATE', [trx_id]);
        if (transactions.length === 0) {
            // Mungkin transaksi sudah diproses atau tidak ada
            console.log(`Callback untuk trx_id ${trx_id} tidak ditemukan atau sudah selesai.`);
            return res.status(200).json({ message: 'OK' });
        }
        const tx = transactions[0];

        if (status.toUpperCase() === 'SUCCESS') {
            // Jika sukses, ubah statusnya saja
            await connection.query('UPDATE transactions SET status = "Success" WHERE id = ?', [tx.id]);
            // Kirim notifikasi ke pengguna di sini (misalnya via WebSocket atau email)
        } else if (status.toUpperCase() === 'FAILED' || status.toUpperCase() === 'REFUNDED') {
            // Jika gagal, ubah status dan kembalikan saldo
            ...
            // Catat pengembalian dana di riwayat saldo
            const historyDesc = `Pengembalian dana untuk invoice ${tx.invoice_id} karena: ${message}`;
...
app.post('/h2h/order', protectH2H, async (req, res) => {
    ...
    try {
        ...
        // Ambil data produk & harga dasarnya
        ...
        // Ambil saldo & role pengguna H2H (datanya sudah ada di h2hUser)
        ...
        // Hitung ulang harga final berdasarkan role user H2H
