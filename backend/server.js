...
app.post('/api/deposit/request', protect, async (req, res) => {
    ...
    try {
        ...
        // SECURITY: Use crypto.randomBytes and crypto.randomInt for secure, unpredictable identifiers instead of Date.now() or Math.random()
...
app.post('/api/admin/balance/add', protectAdmin, async (req, res) => {
    ...
    try {
        ...
        // SECURITY: Use crypto.randomBytes and crypto.randomInt for secure, unpredictable identifiers instead of Date.now() or Math.random()
        await client.query(historySql, [user.id, parseInt(amount), 'Deposit', description, `ADMIN_ADD-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`]);
        await client.query('COMMIT');
...
app.post('/api/admin/balance/reduce', protectAdmin, async (req, res) => {
    ...
    try {
        ...
        const historySql = 'INSERT INTO balance_history (user_id, amount, type, description, reference_id) VALUES ($1, $2, $3, $4, $5)';
        // SECURITY: Use crypto.randomBytes and crypto.randomInt for secure, unpredictable identifiers instead of Date.now() or Math.random()
        await client.query(historySql, [user.id, -amount, 'Refund', description, `ADMIN_REDUCE-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`]);
        await client.query('COMMIT');
...
app.post('/api/order', protect, async (req, res) => {
    ...
    try {
        ...
        // SECURITY: Use crypto.randomBytes and crypto.randomInt for secure, unpredictable identifiers instead of Date.now() or Math.random()
        const invoiceId = `TRX-${Date.now()}-${crypto.randomBytes(4).toString('hex')}${userId}`;
        const trx_id_provider = `WEB-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`;
...
app.post('/h2h/order', protectH2HIp, protectH2H, async (req, res) => {
    ...
    try {
        ...
        // SECURITY: Use crypto.randomBytes and crypto.randomInt for secure, unpredictable identifiers instead of Date.now() or Math.random()
        const invoiceId = `H2H-${Date.now()}-${crypto.randomBytes(4).toString('hex')}${h2hUser.id}`;
        const trx_id_provider = `H2H-PROVIDER-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`;
